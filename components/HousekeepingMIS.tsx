...
import { InventoryItem, PurchaseOrder, Requisition, UserRole, Contract, Issuance, Category } from '../types';
import { 
  LayoutDashboard, 
  Package, 
  ShoppingCart, 
...
interface HousekeepingMISProps {
...
const HousekeepingMIS: React.FC<HousekeepingMISProps> = ({
  inventory,
  purchaseOrders,
  requisitions,
  categories,
  ...
  const hkInventory = inventory.filter(item => 
    ['Linen', 'Cleaning', 'Housekeeping', 'Sanitation', 'HOUSEKEEPING ITEMS'].includes(item.category)
  );

  const hkPOs = purchaseOrders.filter(po => {
     const firstItem = po.items[0];
     const invItem = inventory.find(i => i.name === firstItem.item_name);
     return invItem ? ['Linen', 'Cleaning', 'Housekeeping', 'Sanitation', 'HOUSEKEEPING ITEMS'].includes(invItem.category) : true;
  });

  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { id: 'inventory', label: 'Linen & Cleaning', icon: Package },
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'dashboard': return <StoreDashboard inventory={hkInventory} purchaseOrders={hkPOs} requisitions={requisitions} repairs={[]} contracts={[]} issuances={[]} />;
      case 'inventory': return <InventoryList items={hkInventory} onAddItem={onAddItem} onUpdateItem={onUpdateItem} onDeleteItem={activeRole === UserRole.ADMIN ? onDeleteItem : undefined} categories={categories} />;
      case 'procurement': return <Procurement orders={hkPOs} inventory={hkInventory} onCreateOrder={onCreateOrder} />;
      case 'requisitions': return <Requisitions requisitions={requisitions} inventory={hkInventory} isAdmin={true} onAddRequisition={onAddRequisition} />;
      case 'vendors': return <Vendors />;
      case 'categories': return <Categories categories={categories} inventory={hkInventory} onAddCategory={onAddCategory} onUpdateCategory={onUpdateCategory} onDeleteCategory={onDeleteCategory} />;
      default: return <StoreDashboard inventory={hkInventory} purchaseOrders={hkPOs} requisitions={requisitions} repairs={[]} contracts={[]} issuances={[]} />;
    }
  };
    ...
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans">
      ...
      `}>
         <div className="h-40 flex flex-col items-center justify-center px-6 bg-cyan-950 border-b border-cyan-900">
            ...
            <div className="text-center">
               ...
               <span className="block text-[10px] text-cyan-400 uppercase tracking-widest mt-1 font-bold">Housekeeping</span>
            </div>
         </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
         <header className="lg:hidden bg-white border-b border-slate-200 h-16 flex items-center px-4">
            ...
            <span className="ml-4 font-bold text-slate-800">Housekeeping MIS</span>
         </header>
...
export default HousekeepingMIS;
