
import React, { useState } from 'react';
import { 
  InventoryItem, 
  PurchaseOrder, 
  Requisition, 
  Repair, 
  UserRole, 
  Contract, 
  Issuance, 
  Category
} from '../types';
import { 
  LayoutDashboard, 
  Package, 
  ShoppingCart, 
  ClipboardList, 
  Wrench, 
  Users, 
  Tags, 
  Building2, 
  LogOut,
  ArrowLeft,
  Menu
} from 'lucide-react';
import StoreDashboard from './StoreDashboard';
import InventoryList from './InventoryList';
import Procurement from './Procurement';
import Requisitions from './Requisitions';
import Repairs from './Repairs';
import IssueItems from './IssueItems';
import Vendors from './Vendors';
import Categories from './Categories';
import { MOCK_STAFF } from '../constants';

interface StoreMISProps {
  inventory: InventoryItem[];
  purchaseOrders: PurchaseOrder[];
  requisitions: Requisition[];
  repairs: Repair[];
  contracts: Contract[];
  issuances: Issuance[];
  categories: Category[];
  onAddItem: (item: InventoryItem) => void;
  onUpdateItem: (item: InventoryItem) => void;
  onDeleteItem: (id: string) => void;
  onCreateOrder: (po: PurchaseOrder) => void;
  onAddRequisition: (req: Requisition) => void;
  onAddRepair: (repair: Repair) => void;
  onIssueItem?: (issuance: Issuance) => void;
  onAddCategory: (category: Category) => void;
  onUpdateCategory: (category: Category) => void;
  onDeleteCategory: (categoryId: string) => void;
  activeRole: UserRole;
  onBack: () => void;
}

const StoreMIS: React.FC<StoreMISProps> = ({
  inventory,
  purchaseOrders,
  requisitions,
  repairs,
  contracts,
  issuances,
  categories,
  onAddItem,
  onUpdateItem,
  onDeleteItem,
  onCreateOrder,
  onAddRequisition,
  onAddRepair,
  onIssueItem,
  onAddCategory,
  onUpdateCategory,
  onDeleteCategory,
  activeRole,
  onBack
}) => {
  const [currentView, setCurrentView] = useState('dashboard');
  const [isSidebarOpen, setSidebarOpen] = useState(false);

  // Dedicated Sidebar Items for Store MIS
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { id: 'inventory', label: 'Inventory', icon: Package },
    { id: 'issue_items', label: 'Issue Items', icon: Users },
    { id: 'requisitions', label: 'Store Requisitions', icon: ClipboardList },
    { id: 'procurement', label: 'Purchase Orders', icon: ShoppingCart },
    { id: 'repairs', label: 'Repairs', icon: Wrench },
    { id: 'vendors', label: 'Vendors', icon: Building2 },
    { id: 'categories', label: 'Categories', icon: Tags },
  ];

  // Dummy handler for local state if prop missing (during transition)
  const handleIssueItem = onIssueItem || ((i: Issuance) => console.log('Issue item', i));

  const renderContent = () => {
    switch (currentView) {
      case 'dashboard':
        return (
          <StoreDashboard 
            inventory={inventory} 
            purchaseOrders={purchaseOrders} 
            requisitions={requisitions} 
            repairs={repairs} 
            contracts={contracts} 
            issuances={issuances}
          />
        );
      case 'inventory':
        return (
          <InventoryList 
            items={inventory} 
            onAddItem={onAddItem} 
            onUpdateItem={onUpdateItem} 
            onDeleteItem={activeRole === UserRole.ADMIN ? onDeleteItem : undefined} 
            categories={categories} 
          />
        );
      case 'issue_items':
        return (
          <IssueItems 
             issuances={issuances} 
             inventory={inventory} 
             staff={MOCK_STAFF} 
             onIssueItem={handleIssueItem} 
          />
        );
      case 'requisitions':
        return <Requisitions requisitions={requisitions} inventory={inventory} isAdmin={true} onAddRequisition={onAddRequisition} />;
      case 'procurement':
        return <Procurement orders={purchaseOrders} inventory={inventory} onCreateOrder={onCreateOrder} />;
      case 'repairs':
        return <Repairs repairs={repairs} inventory={inventory} onAddRepair={onAddRepair} />;
      case 'vendors':
        return <Vendors />;
      case 'categories':
        return <Categories 
          categories={categories} 
          inventory={inventory} 
          onAddCategory={onAddCategory}
          onUpdateCategory={onUpdateCategory}
          onDeleteCategory={onDeleteCategory}
        />;
      default:
        return <StoreDashboard inventory={inventory} purchaseOrders={purchaseOrders} requisitions={requisitions} repairs={repairs} contracts={contracts} issuances={issuances} />;
    }
  };

  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans">
      
      {/* Mobile Sidebar Overlay */}
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-20 lg:hidden"
          onClick={() => setSidebarOpen(false)}
        />
      )}

      {/* Dedicated Store Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-30 w-72 bg-cyan-950 text-slate-300 transform transition-transform duration-300 ease-in-out border-r border-cyan-900
        lg:translate-x-0 lg:static lg:inset-0 flex flex-col shadow-2xl
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
         {/* Sidebar Header */}
         <div className="h-40 flex flex-col items-center justify-center px-6 bg-cyan-950 border-b border-cyan-900">
            <div className="p-2 bg-white/10 rounded-xl mb-3">
               <svg viewBox="0 0 100 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-10 h-8">
                  <path d="M50 75C50 75 10 50 10 25C10 12 20 5 32 5C42 5 50 15 50 15" stroke="#ef4444" strokeWidth="8" strokeLinecap="round"></path>
                  <path d="M50 75C50 75 90 50 90 25C90 12 80 5 68 5C58 5 50 15 50 15" stroke="#10b981" strokeWidth="8" strokeLinecap="round"></path>
                  <path d="M30 40H42L46 25L54 55L58 40H70" stroke="#ef4444" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"></path>
               </svg>
            </div>
            <div className="text-center">
               <span className="block text-xl font-extrabold text-white tracking-wide">PULSE IMS</span>
               <span className="block text-[10px] text-cyan-400 uppercase tracking-widest mt-1 font-bold">Store Console</span>
            </div>
         </div>

         {/* Navigation */}
         <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 scrollbar-thin scrollbar-thumb-cyan-800">
            {menuItems.map(item => (
               <button
                 key={item.id}
                 onClick={() => {
                   setCurrentView(item.id);
                   setSidebarOpen(false);
                 }}
                 className={`
                   flex items-center w-full px-4 py-3.5 rounded-xl text-sm font-medium transition-all duration-200
                   ${currentView === item.id 
                     ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/50' 
                     : 'text-cyan-100 hover:text-white hover:bg-cyan-900'}
                 `}
               >
                  <item.icon className={`w-5 h-5 mr-3 ${currentView === item.id ? 'text-white' : 'text-cyan-400'}`} />
                  {item.label}
               </button>
            ))}
         </nav>

         {/* Footer / Back Button */}
         <div className="p-4 bg-cyan-950 border-t border-cyan-900">
            <button 
              onClick={onBack}
              className="flex items-center w-full px-4 py-3 text-cyan-200 hover:text-white hover:bg-indigo-600/20 border border-transparent hover:border-indigo-500/30 rounded-xl transition-all text-sm font-medium"
            >
               <ArrowLeft className="w-5 h-5 mr-3" />
               Back to Main Menu
            </button>
         </div>
      </aside>

      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
         {/* Top Header for Mobile Menu Trigger */}
         <header className="lg:hidden bg-white border-b border-slate-200 h-16 flex items-center px-4">
            <button onClick={() => setSidebarOpen(true)} className="text-slate-600">
               <Menu className="w-6 h-6" />
            </button>
            <span className="ml-4 font-bold text-slate-800">Store MIS</span>
         </header>

         <main className="flex-1 overflow-y-auto p-4 lg:p-8 bg-slate-100/50">
            <div className="max-w-7xl mx-auto space-y-6">
               {renderContent()}
            </div>
         </main>
      </div>

    </div>
  );
};

export default StoreMIS;
